...
class AttentionQKV(Model):
    ...
    def call(self, queries, keys, values, mask=None):
        ...
        similarity = tf.matmul(queries,keys,transpose_b=True)/tf.sqrt(key_dim) # Compute the similarity according to the QKV formula
...
class MultiHeadProjection(Model):
    ...
    def _split_heads(self, tensor):
        ...
        new_feature_size = tf.cast(feature_size/self.n_heads,tf.int32) # Compute what the feature size per head is.
        # Reshape this projection tensor so that it has n_heads, each of new_feature_size
        """
        tensor_list=[]
        for i in range(self.n_heads):
            tensor_i=tensor[:,:,new_feature_size*i:new_feature_size*(i+1)]
            tensor_list.append(tensor_i)
        tensor=tf.stack(tensor_list,axis=1)
        """
        #tensor=tf.reshape(tensor,[batch_size,self.n_heads,tensorlen,new_feature_size])
